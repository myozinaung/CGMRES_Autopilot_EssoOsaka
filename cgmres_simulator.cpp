...
void nmpcsim::simulation(ContinuationGMRES& nmpc_solver, const double* initial_state_vec, const double start_time, const double end_time, const double sampling_period, const std::string savefile_name)
{
    ...
    std::ofstream conditions_data(save_dir_name + "/" + savefile_name + "_conditions.txt");
    ...
    for(double current_time=start_time; current_time<end_time; current_time+= sampling_period){
        ...
        std::cout << current_time   << ","
                << current_state_vec[0] << "," << current_state_vec[1] << ","
                << current_state_vec[2] << "," << current_state_vec[3] << ","
                << current_state_vec[4] << "," << current_state_vec[5] << ","
                << control_input_vec[0] << "," << nmpc_solver.getError(current_time, current_state_vec) << std::endl;
    ...
    std::cout << "CPU time for per control update: " << total_time/((int)( (end_time-start_time)/(sampling_period)))*1000 << " [mili-sec]" << std::endl;
    ...
    conditions_data << "CPU time for per control update: " << total_time/((int)( (end_time-start_time)/(sampling_period)))*1000 << " [mili-sec]\n";
